...
class InventoryItem {
  constructor(item) {
    ...
    this.titleOrPayoff = item.column_values.find(
      (val) => val.id == inventoryBoardColumns.titleOrPayoff,
    ).text
    this.titleStatus = item.column_values.find(
      (val) => val.id == inventoryBoardColumns.titleStatus,
    ).text
    this.payoffAmount = item.column_values.find(
      (val) => val.id == inventoryBoardColumns.payoffAmount,
    ).text
    this.goodTillDate = item.column_values.find(
      (val) => val.id == inventoryBoardColumns.goodTillDate,
    ).text
    this.lienHolder = item.column_values.find(
      (val) => val.id == inventoryBoardColumns.lienHolder,
    ).text
    ...
    this.attachments = item.column_values.find(
      (val) => val.id == inventoryBoardColumns.attachments,
    ).text
...
export const useMondayStore = defineStore('monday', () => {
  ...
  async function updatePrio(itemId, newValue) {
    await fetch(
      `https://bad76-service-32093452-c0a7fb28.us.monday.app/update-prio?itemId=${itemId}&newValue=${newValue}`,
    ).catch((err) => console.log('Item Load Error:', err))
  }

  return { loading, allItems, itemCount, storeOptions, getItems, getItemsByStore, updatePrio }
})
